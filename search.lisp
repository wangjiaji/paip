(defvar fail nil "Indicates failure for tree-search")
...
(defun breath-first-search (start goal-p successors)
...
(defun price-is-right (price)
  "High penalty for going over price"
  #'(lambda (x) (if (> x price)
...
(defstruct (city (:type list)) name lng lat)
...
(defstruct (path (:print-function print-path))
  state previous (cost-so-far 0) (total-cost 0))
...
(defun map-path (fn path)
  "Call fn on every city on the path, collect the results"
  (if (null path)
      nil
...
(defun iter-wide-search (start goal-p successors cost-fn &key (width 1) (max 100))
  "Beam search, increasing the beam width from width to max"
...
(defun new-states (states successors state= old-states)
  "Generate successive states that has not been seen before"
  ...
  (cond ((null paths) fail)
	((funcall goal-p (path-state (first paths)))
 	...
 	(t (let* ((path (pop paths))
	     ...
	     ;; Update path and old-paths to relect the successors
...
(defun search-n (start goal-p successors cost-fn beam-width n)
  (let ((solution nil))
    (beam-search start #'(lambda (x)
			   (when (funcall goal-p x)
			     (push x solutions))
			   (if (> n (length solutions)) t))
		 successors cost-fn beam-width)
    solutions))
